...
# Constants for credentials and scopes (broader for modify/send/calendar)
...
SCOPES_WORK = SCOPES_GMAIL + ["https://www.googleapis.com/auth/calendar"]
SCOPES_PERSONAL = SCOPES_GMAIL

# Ollama model (updated to a stronger one; adjust as needed)
OLLAMA_MODEL = "gemma3:4b"  # Or "llama2" if preferred
...
# Setup credentials
creds_workspace = setup_credentials(CREDENTIALS_WORK_FILE, SCOPES_WORK, TOKEN_WORK_FILE)
creds_personal = setup_credentials(CREDENTIALS_PERSONAL_FILE, SCOPES_PERSONAL, TOKEN_PERSONAL_FILE)
...
# Gmail tools (with modify scopes; add send/compose tools later if needed)
