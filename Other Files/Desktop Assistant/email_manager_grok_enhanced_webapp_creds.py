...
OLLAMA_MODEL = "gemma3:7b"  # Or "llama2" if preferred
...
def setup_credentials(credentials_file, scopes, token_file, port=8080):
    ...
    if not creds or not creds.valid:
        ...
        creds = flow.run_local_server(port=port, open_browser=False)  # Manual URL visit to avoid conflicts
        with open(token_file, 'w') as token:
            token.write(creds.to_json())
    return creds

# Setup credentials with different ports
