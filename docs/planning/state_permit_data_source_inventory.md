...
| PA | 94/64 | 1 (enumerable) but low-value | PA DEP Bureau of Air Quality | `files.dep.state.pa.us/air/AirQuality/AQPortalFiles/Permits/PermitDocuments/` | Open IIS dir (~3,659 listed; naming `<paFacID>[<permit#>]_Issued_v#.pdf`). BUT no facility name in filenames + permit# ≠ ECHO caa_permit_ids (crosswalk problem), and PA "ethanol" = ECHO false-positives (refineries/chem), not ag. Not the clean ag win hoped. |
| IN | 93/66 | **2** (download ok, targeting blocked) | IDEM | Oracle WebCenter ECM `ecm.idem.in.gov/cs/idcplg` (anonymous) | **Download PROVEN, targeting BLOCKED (2026-06-21).** GET_FILE works; but xSourceID unsearchable, dDocTitle misses current permits, no facility names, FRS crosswalk 503. Acquirer = WIP. Next: CAATS facility search OR full-index OR defer. Recipe+caveats in `reference_idem_oracle_webcenter_permits`. |
