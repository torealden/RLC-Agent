...
-- -----------------------------------------------------------------------------
-- Weather Alerts Raw: Raw weather alerts from API sources
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS bronze.weather_alerts_raw (
    id BIGSERIAL PRIMARY KEY,
    ...
    -- Alert details
    alert_event VARCHAR(200),
    alert_sender VARCHAR(200),
    alert_headline VARCHAR(500),
    ...
    -- Natural key
    UNIQUE (location_id, source, alert_event, alert_start)
);

CREATE INDEX IF NOT EXISTS idx_weather_alerts_location ON bronze.weather_alerts_raw(location_id);
CREATE INDEX IF NOT EXISTS idx_weather_alerts_date ON bronze.weather_alerts_raw(alert_start DESC);
CREATE INDEX IF NOT EXISTS idx_weather_alerts_event ON bronze.weather_alerts_raw(alert_event);

COMMENT ON TABLE bronze.weather_alerts_raw IS 'Raw weather alerts from API sources (OpenWeather, etc.)';
