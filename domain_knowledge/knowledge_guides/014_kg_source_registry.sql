...
-- Extends the KG schema defined in WIRING_PLAN.md (013_knowledge_graph.sql)
-- 
...
CREATE INDEX idx_kg_source_status ON core.kg_source(status);
CREATE INDEX idx_kg_source_type ON core.kg_source(source_type);
CREATE INDEX idx_kg_source_date ON core.kg_source(document_date DESC);
CREATE INDEX idx_kg_source_commodities ON core.kg_source USING GIN(commodities);
CREATE INDEX idx_kg_source_topics ON core.kg_source USING GIN(topics);
...
CREATE TABLE core.kg_provenance (
    id              SERIAL PRIMARY KEY,
...
CREATE INDEX idx_kg_provenance_entity ON core.kg_provenance(entity_type, entity_id);
CREATE INDEX idx_kg_provenance_source ON core.kg_provenance(source_id);


-- ---------------------------------------------------------------------------
-- 3. PROCESSING BATCHES: Group processing runs for tracking
-- ---------------------------------------------------------------------------
CREATE TABLE core.kg_processing_batch (
    id              SERIAL PRIMARY KEY,
...
-- ---------------------------------------------------------------------------
-- 4. ADD PROVENANCE COLUMNS to existing KG tables (from WIRING_PLAN.md)
-- ---------------------------------------------------------------------------
-- These ALTER TABLE statements assume 013_knowledge_graph.sql has already run.
-- They add source tracking directly to the KG tables for quick lookups.
...
CREATE VIEW core.kg_sources_pending AS
...
CREATE VIEW core.kg_source_coverage AS
SELECT 
...
CREATE VIEW core.kg_confidence_ranking AS
SELECT 
    p.entity_type,
    p.entity_id,
    CASE 
    ...
    -- Span in years = how long you've been thinking this way
    EXTRACT(DAYS FROM (MAX(s.document_date) - MIN(s.document_date))) / 365.25 AS span_years
...
CREATE VIEW core.kg_sources_changed AS
SELECT id, source_key, title, content_hash, last_processed
FROM core.kg_source
WHERE status = 'completed'
  AND content_hash IS NOT NULL
ORDER BY last_processed ASC;
-- The processing pipeline compares current hash vs stored hash to detect changes
