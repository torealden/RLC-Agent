...
' =============================================================================
' Connects directly to PostgreSQL database to update grain trade data.
...
Public Sub UpdateGrainData()
    ...
    result = MsgBox("Update " & ActiveSheet.Name & " with the latest 3 months of available data?", _
                    vbYesNo + vbQuestion, "Grain Trade Updater")
...
Public Sub UpdateGrainDataCustom()
    ...
    monthCount = InputBox("How many months of data to update?" & vbCrLf & vbCrLf & _
                          ...
                          "Grain Trade Updater", "3")

    If monthCount = "" Then Exit Sub

    If Not IsNumeric(monthCount) Then
        MsgBox "Please enter a valid number.", vbExclamation, "Grain Trade Updater"
        Exit Sub
    End If
...
Private Sub UpdateFromDatabase(monthCount As Integer)
    ...
    If commodity = "UNKNOWN" Then
        MsgBox "Could not determine commodity from sheet name: " & ws.Name, vbExclamation, "Grain Trade Updater"
        conn.Close
        Application.StatusBar = False
        Application.Cursor = xlDefault
        Exit Sub
    ...
    If rs.BOF And rs.EOF Then
        ...
        MsgBox "No data found for " & commodity & " " & flow & "." & vbCrLf & vbCrLf & _
               ...
               "no trade activity for this commodity/flow.", vbInformation, "Grain Trade Updater"
        Exit Sub
    End If
    ...
    MsgBox "Update complete!" & vbCrLf & vbCrLf & _
           ...
           "Countries not found: " & countriesNotFound, vbInformation, "Grain Trade Updater"
...
QueryError:
    Application.StatusBar = False
    Application.Cursor = xlDefault
    MsgBox "Query error:" & vbCrLf & vbCrLf & Err.Description, vbCritical, "Grain Trade Updater"
